...
- 조회수가 많은 순서대로 짤의 목록 조회  
- 각 짤에는 키워드가 등록되어 있어 같은 키워드의 짤 목록으로 쉽게 이동 가능
<br/><br/>
...
- 회원가입 및 로그인, 로그아웃 기능  
- 로그인 시 로그인 상태 유지 여부를 체크하면 세션이 완료되기 전까지 로그인 상태 유지 가능  
- 마이페이지의 비밀번호 변경 기능  
- 로그인한 회원만 자유게시판에 글과 댓글 작성 가능  
- Spring Security를 통해 회원 관리 기능 구현
<br/><br/>
...
- 상단의 검색창에 키워드를 입력하여 원하는 짤 검색
- 테마별 짤, 상황별 짤 메뉴를 통해서 항목별로 분류되어있는 짤 확인 가능
<br/><br/>
...
- 짤을 클릭하면 이미지의 원본과 키워드 전체를 볼 수 있는 페이지로 이동 
- 이미지 하단의 저장 버튼을 클릭하면 나의 PC에 짤을 쉽게 저장 가능
<br/><br/>
...
- 등록하고자 하는 짤을 첨부하고 그에 맞는 분류코드와 키워드를 입력 가능
<br/><br/>
...
- 관리자만 작성할 수 있는 공지사항 게시판과 회원들이 이용할 수 있는 자유게시판으로 나누어짐
- 기본적으로 CRUD가 가능한 게시판으로 자유게시판은 댓글 작성 가능
<br/><br/>
